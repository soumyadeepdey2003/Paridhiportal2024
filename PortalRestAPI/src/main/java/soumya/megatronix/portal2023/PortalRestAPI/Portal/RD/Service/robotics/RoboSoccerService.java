...
import soumya.megatronix.portal2023.PortalRestAPI.Portal.RD.Repository.robotics.RoboSoccerRepository;
...
@Service
public class RoboSoccerService {
    @Autowired
    private RoboSoccerRepository robotics;

    @Autowired
    private MrdRepository repo;

    @Async
    public CompletableFuture<RoboSoccerModel> roboSoccerRd(RoboSoccerModel member) {
        ...
        if ( gid1.isPresent() &&
            ...
            List<RoboSoccerModel> list = robotics.findAll();
            for (RoboSoccerModel i : list) {
                if (
                        (
            ...
            if(
                ...
                CompletableFuture<RoboSoccerModel> roboSoccer = CompletableFuture.completedFuture(robotics.save(member));
    ...
    public CompletableFuture<MrdModel> checkgid(String gid) {
        MrdModel roboSoccer = repo.getModelByGid(gid);
        if (roboSoccer != null) {
            for (RoboSoccerModel i : robotics.findAll()) {
                if (
                        roboSoccer.getGid().equals(i.getGid1()) ||
                                roboSoccer.getGid().equals(i.getGid2()) ||
                                roboSoccer.getGid().equals(i.getGid3()) ||
                                roboSoccer.getGid().equals(i.getGid4()) ||
                                roboSoccer.getGid().equals(i.getGid5())
                ) {
                    throw new RuntimeException("GID already exists.");
                }
            }

            return CompletableFuture.completedFuture(roboSoccer);
        } else {
            throw new RuntimeException("GID not present");
        }
    }
